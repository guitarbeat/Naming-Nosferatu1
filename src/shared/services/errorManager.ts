...
function logError(
	...
	if (process.env.NODE_ENV === "development") {
		const diag = formattedError.diagnostics as {
			fingerprint?: string;
			environment?: Record<string, unknown>;
			debugHints?: { title: string; detail: string }[];
		};
		const componentStack =
			(metadata.componentStack as string | undefined) ||
			(formattedError.metadata.componentStack as string | undefined) ||
			null;

		console.group(
			`🔴 [${formattedError.severity.toUpperCase()}] ${formattedError.type} error — ${context}`,
		);
		...
		console.table({
			...
			Fingerprint: diag.fingerprint ?? "n/a",
		});
		...
		for (const [k, v] of Object.entries(metadata)) {
			if (k !== "componentStack" && k !== "stack") {
				remainingMeta[k] = v;
			}
		}
		if (Object.keys(remainingMeta).length > 0) {
			console.groupCollapsed("Metadata");
			console.table(remainingMeta);
			console.groupEnd();
		}

		if (diag.environment && Object.keys(diag.environment).length > 0) {
			console.groupCollapsed("Environment");
			console.table(diag.environment);
			console.groupEnd();
		}

		if (diag.debugHints && diag.debugHints.length > 0) {
			console.groupCollapsed("Debug Hints");
			console.table(diag.debugHints);
			console.groupEnd();
		}

		const metaLines = Object.entries(remainingMeta)
			.map(([k, v]) => `  ${k}: ${JSON.stringify(v)}`)
			.join("\n");

		const report = [
			"=== DIAGNOSTIC REPORT ===",
			`Generated : ${formattedError.timestamp}`,
			`ID        : ${formattedError.id}`,
			`Fingerprint: ${diag.fingerprint ?? "n/a"}`,
			"",
			"--- Error ---",
	...
	static setupGlobalErrorHandling(): () => void {
		...
		const h = (e: ErrorEvent | PromiseRejectionEvent) => {
			...
			const eventMeta: Record<string, unknown> = {
				isCritical: true,
				eventType: isRejection ? "unhandledrejection" : "error",
			};
			if (isRejection) {
				error = (e as PromiseRejectionEvent).reason;
			} else {
				const ev = e as ErrorEvent;
				error = ev.error ?? ev.message ?? ev;
				if (ev.filename) {
					eventMeta.filename = ev.filename;
				}
				if (ev.lineno) {
					eventMeta.lineno = ev.lineno;
				}
				if (ev.colno) {
					eventMeta.colno = ev.colno;
				}
			}
			ErrorManager.handleError(error, "Global", eventMeta);
		};
